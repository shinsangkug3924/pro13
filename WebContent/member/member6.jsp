...
%> 
    ...
    <%--2. 요청한 값 얻기(가입을 위해 입력한 값들 request에서 얻기)
    	...
    	회원가입창(memberForm.html)에서 요청한 모든 값들을 request영역에서 모두 꺼내어서(얻어서)
    	MemberBean객체의 모든 setter메소드들을 호출해서  모든 인스턴스변수에 각각 저장시키자
    	
    	property="*" 를 작성하는 조건 : <input>태그의 name속성값이름과  MemberBean의 각 변수이름이 모두 동일해야합니다.
     --%>	
